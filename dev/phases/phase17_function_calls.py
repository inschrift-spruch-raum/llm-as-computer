...
def test_invariants():
    ...
    checks.append(ok)
    print(
        f"  {'PASS' if ok else 'FAIL'}  Locals isolation (caller={np_trace.steps[-1].top}, expected=10)"
    )
    ...
    checks.append(ok)
    print(
        f"  {'PASS' if ok else 'FAIL'}  fact(5)={np_trace.steps[-1].top} (expected 120)"
    )
    ...
    checks.append(ok)
    print(
        f"  {'PASS' if ok else 'FAIL'}  fact(10)={np_trace.steps[-1].top} (expected 3628800)"
    )
    ...
    checks.append(ok)
    print(
        f"  {'PASS' if ok else 'FAIL'}  RETURN without CALL traps (np={np_trapped}, pt={pt_trapped})"
    )
