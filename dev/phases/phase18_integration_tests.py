...
def test_bubble_sort_3():
    ...
    prog = [
        ...
        Instruction(OP_I32_STORE),  # 0-2
        Instruction(OP_PUSH, 1),
        Instruction(OP_PUSH, 1),
        Instruction(OP_I32_STORE),  # 3-5
        Instruction(OP_PUSH, 2),
        Instruction(OP_PUSH, 2),
        Instruction(OP_I32_STORE),  # 6-8
...
def test_array_sum_via_function():
    ...
    prog = [
        ...
        Instruction(OP_I32_STORE),  # 9-11
        Instruction(OP_PUSH, 4),
        Instruction(OP_PUSH, 50),
        Instruction(OP_I32_STORE),  # 12-14
...
def test_multi_function_max():
    ...
    prog = [
        ...
        Instruction(OP_PUSH, 0),
        Instruction(OP_PUSH, 7),
        Instruction(OP_I32_STORE),  # 0-2
        Instruction(OP_PUSH, 1),
        Instruction(OP_PUSH, 3),
        Instruction(OP_I32_STORE),  # 3-5
        Instruction(OP_PUSH, 2),
        Instruction(OP_PUSH, 9),
        Instruction(OP_I32_STORE),  # 6-8
        Instruction(OP_PUSH, 3),
        Instruction(OP_PUSH, 1),
        Instruction(OP_I32_STORE),  # 9-11
...
def test_step_comparison():
    ...
    print(
        f"  Loop fib(10):      {loop_steps:>6} steps, result={loop_trace.steps[-1].top}"
    )
    print(
        f"  Recursive fib(10): {rec_steps:>6} steps, result={rec_trace.steps[-1].top}"
    )
...
def test_architecture_summary():
    ...
    total_params = (
        sum(sum(p.numel() for p in h.parameters()) for _, h in heads)
        + model.M_top.numel()
        + model.sp_deltas.numel()
    )
...
def test_trace_match():
    ...
    for name, test_fn in INTEGRATION_TESTS:
        ...
        if not match:
            all_ok = False
        print(
            f"  {status}  {name}: {len(np_trace.steps)} steps, {'match' if match else detail}"
        )
