...
| `src/llm_as_computer/executor.py` | 1070 | NumPyExecutor（NumPy 后端）、CompiledModel（PyTorch nn.Module）、TorchExecutor（PyTorch 后端） |
...
| 文件 | 行数 | 说明 |
|------|------|------|
| `src/llm_as_computer/assembler.py` | 225 | WASM 风格结构化控制流编译器（block/loop/if/br/br_table → 扁平 ISA） |
| `src/llm_as_computer/wat_parser.py` | 712 | WebAssembly 文本格式（WAT）解析器，完整支持 WAT 语法 |
| `src/llm_as_computer/c_pipeline.py` | 635 | C → WAT → ISA 编译管线（需要 clang + wasm2wat） |
